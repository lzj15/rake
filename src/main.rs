...
fn update(state: &mut AppState, message: Message) {
    match message {
        ...
        Message::MovePluginUp(id) => {
            let index = state.added_plugins.iter().position(|p| p.id == id);
            if let Some(i) = index {
                if i > 0 {
        ...
        Message::MovePluginDown(id) => {
            let index = state.added_plugins.iter().position(|p| p.id == id);
            if let Some(i) = index {
                if i < state.added_plugins.len() - 1 {
        ...
        while let Some(command) = self.command_receiver.try_pop() {
            match command {
                ...
                Command::MovePluginUp(id) => {
                    if let Some(index) = self.enabled_plugins.iter().position(|&x| x == id) {
                        if index > 0 {
                ...
                Command::MovePluginDown(id) => {
                    if let Some(index) = self.enabled_plugins.iter().position(|&x| x == id) {
                        if index < self.enabled_plugins.len() - 1 {
